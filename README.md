...
- [rainow/dsh-simple-wiki-memory](https://github.com/rainow/dsh-simple-wiki-memory) — Super-simplified LLM-wiki memory: one auto-injected index document plus one Markdown file per topic read on demand; pending→promote→archive admission with an append-only log and git auto-backup, plain Markdown shared across harnesses.
### Tools & Capabilities
